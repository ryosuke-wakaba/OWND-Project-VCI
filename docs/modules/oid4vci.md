...
RFC 9449では、DPoP-boundなAccess Tokenの実装方式として以下の2つが定義されている。

| 方式 | Access Token形式 | jkt（JWK Thumbprint）の格納場所 | 検証方法 |
|------|-----------------|-------------------------------|----------|
| JWT方式 | JWT | トークン内の`cnf.jkt`クレーム | トークン自己検証 |
| Opaque方式 | ランダム文字列 | サーバー側DB | DB参照 / イントロスペクション |

#### 本ライブラリの実装: Opaque方式

本ライブラリでは**Opaque方式**を採用している。
...
| 観点 | JWT方式 | Opaque方式（本実装） |
|------|---------|---------------------|
| 外部リソースサーバー | 自己検証可能 | イントロスペクション必要 |
| 同一サーバー（OID4VCI） | どちらでも可 | どちらでも可 |
| トークンサイズ | 大きい | 小さい |
| 実装の複雑さ | JWT署名が必要 | シンプル |

OID4VCIでは認可サーバー（Token Endpoint）とリソースサーバー（Credential Endpoint）が同一であるため、Opaque方式で問題なく動作する。
